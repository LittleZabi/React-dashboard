...
export default ({ title, materials }: any) => {
    ...
    useEffect(() => {
        if (modal.sell) {
            setSelectedMat(modal.sell.Material)
            setEditMode(true)
            setFormData(modal.sell)
        }
    }, [])
    ...
    const submitFormData = async (formData: any) => {
        ...
        try {
            ...
            const response = await axios.post(BACKEND_API + '/api/sells/new', { ...formData, ...o });
    ...
    const handleNumberInput = (value: any, field: string) => {
        ...
        setFormData({ ...formData, [field]: n ? n : 1 })
    }
    ...
    return <Modal title={editMode ? 'Edit sell record' : title}>
        <form className="max-w-sm mx-auto" onSubmit={handleSubmit}>
            <div className="mt-2 mb-3 dark:text-white ">
                <Dropdown className="shadow" label={selectedMat ? selectedMat.name : 'Materials'} dismissOnClick={false} style={{ background: 'rgba(28, 100, 242, 1)', width: 170 }} inline >
                    {
            ...
            <p id="helper-text-explanation" className="mt-2 mb-2 text-sm text-gray-500 dark:text-gray-400">Please select quantity of the material which you sell.</p>
            <label htmlFor="quantity-input" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Selling Price:</label>
            <div className="relative flex items-center max-w-[8rem]">
                <button onClick={() => setFormData({ ...formData, sellingPrice: formData.sellingPrice > 0 ? formData.sellingPrice - 1 : formData.sellingPrice })} type="button" id="decrement-button" data-input-counter-decrement="quantity-input" className="bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 border border-gray-300 rounded-s-lg p-3 h-11 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
                    <svg className="w-3 h-3 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 2">
                        <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M1 1h16" />
                    </svg>
                </button>
                <input type="text" value={Number(formData.sellingPrice)} onChange={(e: any) => handleNumberInput(e.target.value, 'sellingPrice')} id="quantity-input" data-input-counter aria-describedby="helper-text-explanation" className="bg-gray-50 border-x-0 border-gray-300 h-11 text-center text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="12" required />
                <button onClick={() => setFormData({ ...formData, sellingPrice: Number(formData.sellingPrice) + 1 })} type="button" id="increment-button" data-input-counter-increment="quantity-input" className="bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 border border-gray-300 rounded-e-lg p-3 h-11 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
                    <svg className="w-3 h-3 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 18">
                        <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 1v16M1 9h16" />
                    </svg>
                </button>
            </div>
            <p id="helper-text-explanation" className="mt-2 mb-2 text-sm text-gray-500 dark:text-gray-400">Enter the price of sell in total.</p>
