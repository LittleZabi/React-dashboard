...
export default () => {
    ...
    useEffect(() => {
        Promise.all([
            axios.get(BACKEND_API + '/api/count/visitors', { params: { id: User.id, asAdmin: User.asAdmin } }),
            axios.get(BACKEND_API + '/api/count/sells', { params: { id: User.id, asAdmin: User.asAdmin } }),
            axios.get(BACKEND_API + '/api/count/user', { params: { id: User.id, asAdmin: User.asAdmin } }),
            axios.get(BACKEND_API + '/api/count/purchases', { params: { id: User.id, asAdmin: User.asAdmin } }),
            axios.get(BACKEND_API + '/api/count/materials/top-materials', { params: { id: User.id, asAdmin: User.asAdmin } })
        ])
            .then(([visitors, sells, users, purchases, materialTop]: any) => {
                ...
                setPurchaseData(PValues.length > 1 ? [PValues[0], PValues[1]] : [[], PValues[PValues.length - 1]]);
                // sells
                let s: any = {}
                ...
                setSellsData(values.length > 1 ? [values[0], values[1]] : [[], values[values.length - 1]]);
                // usrs
                let u: any = {}
        ...
        <div className="d-wrap">
            ...
            <div className=' flex mb-8 '>
                ...
                <Card className='m-1 w-1/3'>
                    <h1 className='text-gray-700 dark:text-white text-base font-medium'>Top materials Purchase</h1>
                    <div className='flex items-center -mt-3 mb-5'>
                        <Icon icon="solar:tag-price-bold" className="mr-2 text-3xl text-pink-500" />
                        <span className='text-xl text-black dark:text-white'>{numberFormatChart(totalPurchase, 0, undefined, ',')}</span>
                    </div>
