...
export default () => {
    ...
    const [sells, setSells]: any = useState({})
    const getData = async () => {
        await axios.get(BACKEND_API + `/api/sells/all/${User.id}/${User.asAdmin}`).then((res) => {
            let data = res.data
    ...
    return <div className="mt-6 user-view">
        ...
        <button onClick={() => setModal({ newSell: true })} className="bg-blue-700 dark:bg-gray-700  focus:ring-4 dark:focus:ring-blue-900 focus:ring-blue-300 hover:bg-gray-900 text-white py-2 px-6 rounded inline-flex items-center">
            <span className="mx-1">
                <Icon icon="ic:twotone-plus" /></span>
            <span>New Sell</span>
        </button>
        {modal.newSell && <Sell materials={materials} title={"Add new sell"} />}
        {modal.confirmation &&
